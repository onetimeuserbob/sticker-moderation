        ...
        return [
            {
                ...
                "text": (
                    ...
                    "OUTPUT FORMAT IS PLAIN TEXT — DO NOT use Markdown of any "
                    "kind. No asterisks for bold (`*x*` or `**x**`), no "
                    "underscores for italics (`_x_`), no backticks for code, "
                    "no `#` headers, no `[label](url)` links. Telegram will "
                    "show those characters literally and it looks broken. "
                    "Use blank lines, UPPERCASE, and emoji for emphasis "
                    "instead. When you reproduce the READY SUMMARY below, "
                    "send it AS-IS without adding markdown around it. When "
                    "the user proposes a rule change, USE THE TOOLS — don't "
                    "just say you'll add it; actually add it. After a tool "
                    "call, briefly confirm what you did.\n\n"
